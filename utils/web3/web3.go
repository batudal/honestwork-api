...
func FetchUserNFT(address string) int {
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "ethClientDialError")
	}
	defer client.Close()
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "honestworknftNewHonestworknftError")
	}
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "honestworknftTokenOfOwnerByIndexError")
	}
...
func FetchUserState(address string) int {
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "configParseConfigError")
	}
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "honestworknftGetUserTierError")
	}
	client.Close()
...
func FetchTokenTier(token_id int) int {
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "honestworknftGetTokenTierError")
	}
	client.Close()
	return int(state.Int64())
}
...
func FetchNFTRevenue(network_id int, token_id int) int {
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "hwregistryNewHwregistryError")
	}

	revenue, err := instance.GetNFTGrossRevenue(nil, big.NewInt(int64(token_id)))
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "hwregistryGetNFTGrossRevenueError")
	}
...
func FetchTotalSupply() int {
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "honestworknftTotalSupplyError")
	}
	client.Close()
...
func CheckOutstandingPayment(user_address string, token_address string, amount *big.Int, tx_hash string) error {
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "ethClientTransactionByHashError")
	}
	if isPending {
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "ethClientTransactionReceiptError")
	}
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "ethClientFilterLogsError")
	}
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "abiJSONError")
	}
	...
	for _, v_log := range logs {
		...
		if err != nil {
			loggersentry.InitSentry()
			loggersentry.CaptureErrorMessage(err.Error() + "abiUnpackError")
		}

		payment_amount := payment_event[0].(*big.Int)
		if payment_amount.Cmp(amount) != 0 {
			loggersentry.InitSentry()
			loggersentry.CaptureErrorMessage("amount paid mismatch")
		...
		if token_addr != token_address {
			loggersentry.InitSentry()
			loggersentry.CaptureErrorMessage("token address mismatch")
...
func CheckNFTOwner(user_address string, token_address string, token_id int) bool {
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "ethClientNewHonestworknftError")
	}
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "ethClientOwnerOfError")
	}
...
func CheckENSOwner(user_address string, ens_name string) bool {
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "ensResolveError")
		return false
	}
	if address.String() != user_address {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage("ens address mismatch")
		return false
	}
	return true
}
...
func FetchAggregatedRating(user_address string) float64 {
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "ethClientNewHwescrowError")
	}
	...
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "ethClientGetAggregatedRatingError")
	}

	precision, err := instance.GetPrecision(nil)
	if err != nil {
		loggersentry.InitSentry()
		loggersentry.CaptureErrorMessage(err.Error() + "ethClientGetPrecisionError")
	}
